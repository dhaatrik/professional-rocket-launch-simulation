...
## 2026-04-19 - Strengthen IPC Validation
**Vulnerability:** Missing structural validation on IPC messages.
**Learning:** Relying solely on origin checks for BroadcastChannels and Web Workers leaves the application vulnerable to malformed payloads or data injection from within the same origin.
**Prevention:** Enforce structural and type validation on the `MessageEvent.data` payload before processing the message downstream.
