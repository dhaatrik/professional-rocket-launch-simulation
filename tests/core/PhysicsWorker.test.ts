...
describe('PhysicsWorker', () => {
    ...
    it('should handle FC_START, FC_PAUSE, and FC_STOP commands', () => {
        workerOnMessage!({
            data: {
                type: 'INIT',
                payload: {}
        ...
        // Load a script first, otherwise it won't activate
        workerOnMessage!({
            data: {
                type: 'COMMAND',
                payload: { type: 'FC_LOAD_SCRIPT', script: 'WHEN ALTITUDE > 1000 THEN PITCH 45' }
        ...
        workerOnMessage!({
            data: {
                type: 'COMMAND',
                payload: { type: 'FC_START' }
            }
        } as MessageEvent);

        // Trigger a step to let worker post state with updated FC status
        workerOnMessage!({
            data: {
                type: 'STEP',
                payload: { dt: 0.02, timeScale: 1, controls: { throttle: 1, ignition: true } }
